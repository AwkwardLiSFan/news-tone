...
with open('independent/independent_body.csv', 'w', newline='', encoding='utf-8') as file:
    fieldnames = ['head','head_sentiment','head_snmnt','body','sentiment_score','sntmnt2']
    writer = csv.DictWriter(file, fieldnames=  fieldnames)
    writer.writeheader()
    for i in range(48):
        writer.writerow({'head': h_data_to_write[i]['article'], 'head_sentiment': h_data_to_write[i]['s_score'], 'head_snmnt': h_data_to_write[i]['sent'], 'body': data_to_write[i]['article'], 'sentiment_score': data_to_write[i]['s_score'], 'sntmnt2': data_to_write[i]['sent']})
